...
An independent source check uses `record_population_manifest` on an open Main period with `period`, `source_name`, `rationale`, `contract_references` (an array of legal source agreement IDs), `billing_references` (an array of `{ "contract_reference": "...", "invoice_reference": "...", "amount": "..." }`), `usage_references` (an array of `{ "contract_reference": "...", "usage_reference": "...", "quantity": "...", "invoice_value": "..." }`), and `opening_positions` (an array of `{ "contract_reference": "...", "cutover_date": "YYYY-MM-01", "billed_to_date": "...", "contract_asset": "...", "deferred_revenue": "...", "recognized_to_date": "..." }`). Supply empty arrays explicitly when the independent extract has no rows. Reports compares the source lists to active or outstanding workspace source agreements, billing entries effective in that month including credits, externally priced metered usage delivered in that month, and opening positions effective on the first day of that month. A combined accounting contract contributes each source agreement to the agreement comparison; an opening position compares under its accounting contract reference because its balance covers the combined contract. Invoice amounts are signed, so credit memos use negative values. Source invoice and opening amounts must be stated in cents. The check compares each source invoice amount, delivered priced-usage units and value, and all four cumulative opening balances with the corresponding workspace record. Older ID-only invoice or usage rows remain review exceptions until values are supplied. Older manifests without an opening list leave actual cutover openings unverified. A billing import's stable `source_id` is used when it has no invoice reference. Missing, unexpected, duplicate, unidentified, unverified, or mismatched records become a separate close review item and appear in the workbook, including the Source invoices, Source priced usage, and Source openings sheets. The comparison is captured in the close checkpoint; the source extract and population filters still need the accountant's review. Agreements need stable references to match the register; invoice references or stable imported source IDs identify billing rows.
