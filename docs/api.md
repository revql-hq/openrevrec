...
Scenario rebase can combine two additive billing facts on the same contract when both have distinct invoice or credit identities. Two credits must refer to different earlier invoices by the same original-identity type; workspace originals must also have distinct source invoice identities. A credit against an earlier workspace or external invoice can combine with a new positive invoice only when their invoice identities differ. Two corrections to different positive invoices can also combine when the original and replacement invoice identities are distinct across branches. Duplicate identities, a credit against the newly added invoice, corrections to the same invoice, replacement-identity collisions, and unlike original-identity types remain conflicts for manual review. Rebase does not treat a correction to progress, usage, or rates, or a contract or policy change, as an independent invoice.
...
An independent source check uses `record_population_manifest` on an open Main period with `period`, `source_name`, `rationale`, `contract_references` (an array of legal source agreement IDs), `billing_references` (an array of `{ "contract_reference": "...", "invoice_reference": "...", "amount": "..." }`), and `usage_references` (an array of `{ "contract_reference": "...", "usage_reference": "...", "quantity": "...", "invoice_value": "..." }`). Supply empty arrays explicitly when the independent extract has no rows. Reports compares the source lists to active or outstanding workspace source agreements, billing entries effective in that month including credits, and externally priced metered usage delivered in that month. A combined accounting contract contributes each of its source agreements to the register comparison, while each invoice or usage record compares under its selected source agreement. Invoice amounts are signed, so credit memos use negative values; each source amount must be stated in cents. The check compares each source invoice amount with its workspace entry, and compares delivered units and cent-precision invoice value for each priced-usage record. Older ID-only invoice or usage rows remain review exceptions until values are supplied. A billing import's stable `source_id` is used when it has no invoice reference. Missing, unexpected, duplicate, unidentified, unverified, or mismatched records become a separate close review item and appear in the workbook, including the Source invoices and Source priced usage sheets. The comparison is captured in the close checkpoint; the source extract and population filters still need the accountant's review. Agreements need stable references to match the register; invoice references or stable imported source IDs identify billing rows.
