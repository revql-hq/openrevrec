...
For a paragraph 17 combined contract, `create_contract` includes `reference` for the primary agreement, `source_contracts: [{"reference":"AG-A","agreement_date":"2026-01-01","customer_id":"parent"},{"reference":"AG-B","agreement_date":"2026-01-02","customer_id":"subsidiary","relationship_rationale":"Subsidiary is controlled by the primary customer."}]`, `combination_basis` (`package`, `interdependent_price`, or `single_obligation`), and `combination_rationale`. The first source reference must equal the contract reference, and its customer must equal the accounting contract's primary `customer_id`. Each source customer must already exist. A different customer requires a source-specific rationale for why it is a related party; the accountant remains responsible for that conclusion. Older source rows without `customer_id` use the primary customer. All source agreements share one accounting schedule; the accountant supplies the combined consideration and obligations. `record_billing` and `record_usage` on this contract require `source_contract_reference` identifying one of its agreements, including when correcting those facts. A workspace-linked credit must use the original invoice's source agreement. The combination conclusion and its source list cannot be edited after creation.
...
`POST /api/preview` accepts the same command envelope and returns `{ "before": ..., "state": ..., "comparison": ..., "frontier": ..., "request_hash": ... }` without persisting the change. `before` is the report being changed. Applying a scenario previews Main before and after acceptance, even when the request originates inside the scenario. Rebasing previews the target scenario. Use the returned `before` report for the financial comparison rather than the report currently displayed by the caller. After reviewing a preview, pass its `frontier` and `request_hash` as `expected_frontier` and `expected_request_hash` on `POST /api/commands`. The command is rejected if any workspace change was recorded or the command details changed since preview; review a fresh preview before retrying. Direct commands may omit these optional fields.

Scenario rebase can combine two additive billing facts on the same contract when both have distinct invoice or credit identities. Two credits must refer to different earlier invoices by the same original-identity type; workspace originals must also have distinct source invoice identities. A credit against an earlier workspace or external invoice can combine with a new positive invoice only when their invoice identities differ. Two corrections to different positive invoices can also combine when the original and replacement invoice identities are distinct across branches. Separately identified usage source rows can also combine; a new usage row and a correction, or corrections to different usage rows, can combine when every original and replacement source identity differs across branches. Cumulative progress and milestone facts, including corrections, can combine when they target different obligations on the same contract. Progress measurements and corrections on the same obligation can also combine when their original and replacement effective dates and source identities do not overlap; the combined cumulative measure is replayed by date. Two milestone changes to the same obligation still conflict. Dated metered rate changes and corrections can combine when their original and replacement effective dates and imported source identities do not overlap; the combined rate history and usage valuation are recalculated. Rebase recalculates the combined contract; finite-unit recognition remains capped at the allocation and excess delivery is flagged for review. Applying still enforces closed-period rules. Unidentified or duplicate usage rows, corrections to the same usage row, replacement-identity collisions, a credit against the newly added invoice, unlike original-identity types, and same-day, same-target, or duplicate-source progress changes remain conflicts for manual review. Rate changes on the same day or to the same correction target, and contract or policy changes, remain subject to conflict review.
...
`link_renewal_contract` accepts the original `contract_id`, exercised-right `obligation_id`, `renewal_contract_id`, `additional_consideration`, `price_basis` (`new_consideration_only`), and rationale. The renewal contract must belong to the same customer, have a date range that covers the exercised right's delivery window, keep every obligation inside that window, and contain only the new transaction price; the submitted amount must equal its initial transaction price. An optional `effective_date` must match the exercise date. Each right and renewal contract can be linked once. The `Renewal Links` import sheet runs after contracts and right exercises and requires a stable `source_id`. The export's `Renewal links` sheet shows both prices and current-month revenue together, while the source contracts' balances remain separate.
...
For agreements combined across related customers, also supply `contract_customers: [{"contract_reference":"AG-A","source_system":"CRM","customer_reference":"PARENT"}, {"contract_reference":"AG-B","source_system":"CRM","customer_reference":"SUB"}]` from the independent register. `source_system` may be omitted when blank. The customer records in the workspace need matching external references and source systems. Close review flags each combined agreement whose source customer is absent, whose workspace customer lacks an external reference, or whose source and workspace customer identities differ. Customer rows for other agreements are optional but are compared when supplied. The Source contracts workbook sheet retains the customer comparison. Matching identifiers does not establish that two customers are related parties; the accountant must review the relationship conclusion separately.
...
To replace a current opening after source reconciliation, use `correct_opening_position` with `target_change_set_id`, a correction `rationale`, and a complete `replacement` in the same shape as `record_opening_position`. The contract and cutover date must stay fixed. The command retains immutable history, recalculates later periods, and requires a judgment review. Reopen affected closed periods before correcting Main. The current accepted opening appears in the workbook; the original remains in change history.
