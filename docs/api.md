...
`set_policy` also accepts full `account_profiles` and `profile_assignments` maps. A profile has a stable ID and `{ "name": "Services", "accounts": { "revenue": "4100" }, "dimensions": { "Department": "Recurring" } }`; assignments map contract IDs to profile IDs. Resolution order is obligation revenue override, contract role override, assigned profile role, then workspace default. Profile dimensions apply to every line of the assigned contract, including opening transfers. Account or dimension remapping of an existing balance requires `account_transition` (`transfer` or `external`). Journal JSON carries `account_profile_id` and `dimensions`; the workbook uses one column per encountered dimension.
...
`create_contract` and `modify_contract` can carry `term_basis` (`fixed`, `cancellable`, or `evergreen`), `term_assessment_rationale`, `term_reassessment_trigger`, and optional `term_review_date`. A nonfixed basis requires the rationale and trigger. The original contract end and current obligation dates represent the assessed accounting term; a later change to the current service dates belongs in a dated modification. The import template has corresponding Contracts and Amendments columns, and the export includes a Term assessments sheet. A due review date becomes a close-review check; it does not change recognition automatically.
...
Consideration and Amendment Consideration sheets accept `allocation_scope`, comma-separated `target_obligation_ids`, and `allocation_rationale`. Leave them blank for relative SSP. Set `allocation_scope` to `specific` only for an eligible variable, usage, or credit component, with at least one current obligation target and a documented conclusion. The export shows both final obligation allocations and component targeting decisions. Period-specific targeting and prospective modifications with specifically allocated components remain unsupported.
