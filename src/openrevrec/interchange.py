...
SHEETS = {
    ...
    "Contract Sources": ["contract_id", "reference", "agreement_date", "customer_id", "relationship_rationale"],
...
def export_bytes(state, review=None):
    ...
    if review:
        ...
        if population:
            ...
            _sheet(book, "Source opening obligations", ["Contract reference", "Cutover date", "Obligation ID", "Comparison",
                                                         ...
                                                         "Activity ID", "Source", "Population basis", "Source obligation reference"],
                   [[item["contract_reference"], item["cutover_date"], item["obligation_id"], item["status"],
                     ...
                     item["activity_id"], population["source_name"], population["rationale"], item.get("source_obligation_reference", "")]
    ...
    _sheet(book, "Contract combinations", ["Accounting contract ID", "Source agreement reference", "Agreement date", "Paragraph 17 basis", "Accounting rationale", "Change set ID", "Source customer ID", "Related-party rationale"],
           [[contract["id"], source["reference"], source["agreement_date"], contract["combination_basis"], contract["combination_rationale"], contract.get("change_set_id", ""), source.get("customer_id", contract["customer_id"]), source.get("relationship_rationale", "")]
