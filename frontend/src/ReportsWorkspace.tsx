...
export type Review = {
  ...
  population_comparison: {
    ...
    unverified_opening_obligation_measures?: [string, string, string][];
    opening_measure_review_rows?: { contract_reference: string; cutover_date: string; obligation_id: string; workspace_measure: string; activity_id: string }[];
    mismatched_opening_obligation_measures?: { contract_reference: string; cutover_date: string; obligation_id: string; source_measure: string; workspace_measure: string; activity_id: string }[];
...
function SourcePopulation({ review, props }: { review: Review; props: ViewProps }) {
  ...
  const [openingObligations, setOpeningObligations] = useState((manifest?.opening_obligations || []).map((item) => `${item.contract_reference} | ${item.cutover_date} | ${item.obligation_id} | ${item.recognized_to_date}${item.measure !== undefined ? ` | ${item.measure}` : ""}`).join("\n"));
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const submit = async (event: React.FormEvent) => {
    event.preventDefault();
    ...
    for (const [index, line] of openingObligations.split(/\r?\n/).entries()) {
      ...
      const divider = line.includes("\t") ? "\t" : "|";
      const parts = line.split(divider).map((part) => part.trim());
      if (![4, 5].includes(parts.length) || !parts.every(Boolean)) {
        setError(`Opening obligation line ${index + 1} needs contract reference | cutover date | obligation ID | recognized to date, with cumulative measure for progress or units.`);
        return;
      }
      openingObligationRows.push({ contract_reference: parts[0], cutover_date: parts[1], obligation_id: parts[2], recognized_to_date: parts[3], ...(parts.length === 5 ? { measure: parts[4] } : {}) });
    }
    setBusy(true);
    try {
  ...
  return <Section title="Source population" subtitle="Compare independent agreements, invoices, priced usage, and cutover openings with workspace records.">
    ...
    {props.state.scenario_id === "main" && !props.state.report.closed ? <form onSubmit={(event) => void submit(event)}>
      ...
      <Field label="Source opening obligation amounts" hint="Paste one line per mapped obligation: accounting contract reference | cutover date | obligation ID | recognized to date | cumulative measure. Include the fifth column for progress, milestone, point-in-time, or finite-unit usage openings; otherwise the measure stays under review. Include zero amounts; source rows should sum to the contract's source opening total."><textarea rows={6} value={openingObligations} onChange={(event) => setOpeningObligations(event.target.value)} /></Field>
