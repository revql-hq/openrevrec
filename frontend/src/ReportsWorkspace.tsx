...
export type Review = {
  ...
  population_manifest?: { change_set_id?: string; source_name: string; rationale: string; contract_references: string[]; billing_references: { contract_reference: string; invoice_reference: string }[]; usage_references?: { contract_reference: string; usage_reference: string; quantity?: string; invoice_value?: string }[] } | null;
...
function SourcePopulation({ review, props }: { review: Review; props: ViewProps }) {
  ...
  return <Section title="Source population" subtitle="Compare independent source IDs with workspace contracts, invoices, and priced usage.">
    ...
    {manifest ? <>
      ...
      {!groups.some(([, items]) => items.length) && !comparison.unidentified_contracts.length && !comparison.unidentified_billings.length && !(comparison.unidentified_usage || []).length && !(comparison.mismatched_usage || []).length && <p className="notice">Every listed source record matches a workspace record.</p>}
