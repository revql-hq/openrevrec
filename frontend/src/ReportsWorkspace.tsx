...
export type Review = {
  ...
  population_comparison: {
    ...
    mismatched_opening_obligations?: { contract_reference: string; cutover_date: string; obligation_id: string; source_obligation_reference?: string; source_amount: string; workspace_amount: string; activity_id: string }[];
...
function SourcePopulation({ review, props }: { review: Review; props: ViewProps }) {
  ...
  const [openingObligations, setOpeningObligations] = useState((manifest?.opening_obligations || []).map((item) => `${item.contract_reference} | ${item.cutover_date} | ${item.obligation_id} | ${item.recognized_to_date}${item.measure !== undefined || item.source_obligation_reference !== undefined ? ` | ${item.measure || ""}` : ""}${item.source_obligation_reference !== undefined ? ` | ${item.source_obligation_reference}` : ""}`).join("\n"));
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const submit = async (event: React.FormEvent) => {
    event.preventDefault();
    ...
    for (const [index, line] of openingObligations.split(/\r?\n/).entries()) {
      ...
      const divider = line.includes("\t") ? "\t" : "|";
      const parts = line.split(divider).map((part) => part.trim());
      if (![4, 5, 6].includes(parts.length) || !parts.slice(0, 4).every(Boolean) || (parts.length === 5 && !parts[4]) || (parts.length === 6 && !parts[5])) {
        setError(`Opening obligation line ${index + 1} needs four columns, plus cumulative measure and optional legacy reference. Leave the measure blank only for a time-based obligation with a legacy reference.`);
        return;
      }
      openingObligationRows.push({ contract_reference: parts[0], cutover_date: parts[1], obligation_id: parts[2], recognized_to_date: parts[3], ...(parts.length >= 5 && parts[4] ? { measure: parts[4] } : {}), ...(parts.length === 6 ? { source_obligation_reference: parts[5] } : {}) });
    }
    setBusy(true);
    try {
