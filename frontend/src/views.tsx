...
export function ContractView(props: ViewProps) {
  ...
  const unlinkedAddedContracts = state.contracts.some((item) => item.id !== contract.id && item.customer_id === contract.customer_id && item.start_date > contract.start_date && !item.cutover_date && item.consideration.every((component) => component.kind === "fixed") && !(state.modification_links || []).some((link) => link.added_contract_id === item.id) && !(state.renewal_links || []).some((link) => link.renewal_contract_id === item.id));
  const modificationSupport = (state.report.modification_links || []).filter((link) => link.contract_id === contract.id || link.added_contract_id === contract.id);
  ...
  return (
    <>
      ...
      >
        ...
        {unlinkedAddedContracts && <Button disabled={pendingOpening} onClick={() => dialog({ type: "modification_link", contractId: contract.id })}>Link added service</Button>}
        <Button
      ...
      {tab === "Allocation" && (
        ...
        >
          ...
          {modificationSupport.length > 0 && <><h3>Separate-contract amendments</h3>{modificationSupport.map((link) => <div key={link.change_set_id} className="notice"><p><Button type="button" onClick={() => props.navigate("Contracts", link.contract_id, "Allocation")}>{link.contract_name}</Button> → <Button type="button" onClick={() => props.navigate("Contracts", link.added_contract_id, "Allocation")}>{link.added_contract_name}</Button> · approved {dateLabel(link.effective_date)}</p><p>Initial added consideration {money(link.initial_additional_consideration, currency)}. This month: original {money(link.original_revenue, currency)} + added {money(link.added_revenue, currency)} = {money(link.combined_revenue, currency)} revenue.</p><p className="fine-print">Each contract keeps its own balance. Original asset {money(link.original_contract_asset, currency)} / deferred {money(link.original_deferred_revenue, currency)}; added asset {money(link.added_contract_asset, currency)} / deferred {money(link.added_deferred_revenue, currency)}. {link.rationale}</p></div>)}</>}
        </Section>
      )}
      {tab === "Recognition" && (
    ...
    <>
      ...
      {Boolean(state.report.account_positions?.length) && <Section title="Projected contract balances by account" subtitle={state.report.runoff_pending?.length ? "This period includes provisional account splits above. Allocate them before close or posting, then reconcile every account to the ledger." : "These balances reflect the recorded account allocations and mapping. Confirm any external transfer in the ledger and reconcile before posting."}>
        <div className="table-wrap"><table><thead><tr><th>Contract</th><th>Role</th><th>Account</th><th>Dimensions</th><th className="number">Closing balance</th></tr></thead><tbody>{state.report.account_positions?.map((item, index) => <tr key={`${item.contract_id}:${item.role}:${item.account}:${index}`}>
          ...
          <td>{Object.entries(item.dimensions).map(([key, value]) => `${key}: ${value}`).join(" · ") || "—"}</td>
          <td className="number">{money(item.balance, state.workspace.currency)}</td>
        </tr>)}</tbody></table></div>
      </Section>}
      <Section title="Posting combination preflight" subtitle={state.report.policy_account_dimension_rules?.length ? `Checked against ${state.report.policy_account_dimension_source}. ${state.report.policy_account_dimension_coverage === "complete" ? "Every journal account must be covered for close." : "Unlisted accounts require close review."} Confirm final posting rules in the destination ledger.` : "No approved combination list is configured for this period."} action={<Button type="button" disabled={state.scenario_id !== "main"} onClick={() => props.dialog({ type: "policy" })}>{state.report.policy_account_dimension_rules?.length ? "Edit approved combinations" : "Add approved combinations"}</Button>}>
        ...
        {Boolean(state.report.account_dimension_unvalidated_accounts?.length) && <p className="fine-print">No supplied combinations for account(s): {state.report.account_dimension_unvalidated_accounts?.join(", ")}. {state.report.policy_account_dimension_coverage === "complete" ? "Add every missing account to the approved list before close." : "Add them to the approved list or document external validation before close."}</p>}
        {Boolean(state.report.policy_account_dimension_rules?.length) && !state.report.account_dimension_exceptions?.length && !state.report.account_dimension_unvalidated_accounts?.length && <p>Every journal line matches a supplied approved combination.</p>}
      </Section>
  ...
  return (
    <>
      ...
      >
        ...
        {Boolean(state.policy.account_dimension_rules?.length) && <><h3>Approved posting combinations</h3><p className="fine-print">Source: {state.policy.account_dimension_source}. {state.policy.account_dimension_coverage === "complete" ? "Every journal account must match this list before close." : "Listed accounts are checked; other accounts remain under review."} The destination ledger remains authoritative.</p><div className="table-wrap"><table><thead><tr><th>Account</th><th>Required dimension combination</th></tr></thead><tbody>{state.policy.account_dimension_rules?.map((rule, index) => <tr key={`${rule.account}:${index}`}><td className="mono">{rule.account}</td><td>{Object.entries(rule.dimensions).map(([key, value]) => `${key}: ${value}`).join(" · ") || "No dimensions"}</td></tr>)}</tbody></table></div></>}
      </Section>
