  ...
  return (
    <>
      ...
      <Section title="Posting combination preflight" subtitle={state.report.policy_account_dimension_rules?.length ? `Checked against ${state.report.policy_account_dimension_source}. Only listed accounts are covered; confirm final posting rules in the destination ledger.` : "No approved combination list is configured for this period."} action={<Button type="button" disabled={state.scenario_id !== "main"} onClick={() => props.dialog({ type: "policy" })}>{state.report.policy_account_dimension_rules?.length ? "Edit approved combinations" : "Add approved combinations"}</Button>}>
        ...
        {Boolean(state.report.account_dimension_unvalidated_accounts?.length) && <p className="fine-print">No supplied combinations for account(s): {state.report.account_dimension_unvalidated_accounts?.join(", ")}. Add them to the approved list or document external validation before close.</p>}
  ...
  return (
    <>
      ...
      >
        ...
        {Boolean(state.policy.account_dimension_rules?.length) && <><h3>Approved posting combinations</h3><p className="fine-print">Source: {state.policy.account_dimension_source}. These exact combinations are checked against journal lines for this period; other accounts remain unvalidated.</p><div className="table-wrap"><table><thead><tr><th>Account</th><th>Required dimension combination</th></tr></thead><tbody>{state.policy.account_dimension_rules?.map((rule, index) => <tr key={`${rule.account}:${index}`}><td className="mono">{rule.account}</td><td>{Object.entries(rule.dimensions).map(([key, value]) => `${key}: ${value}`).join(" · ") || "No dimensions"}</td></tr>)}</tbody></table></div></>}
      </Section>
